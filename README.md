...
* [x] Authored loot tables for selected discoveries
* [ ] Custom blocks
* [ ] Custom entities
* [ ] Environmental hazards
* [ ] Ambient events
...
**The Labrinth is currently in early development:** the `labrinth:labrinth`
dimension uses deterministic 64-block generation cells and a mixed
room-and-corridor catalog that re-materializes content as chunks load, without
retaining a pre-generated layout. Room metadata carries weights, rarity,
rotation, region/depth gates, connectors, placement conditions, decorations,
and loot references. Interiors are materialized chunk-locally with lighting,
props, block variation, visible containers, interactive markers, and capped
unmatched connectors; no neighboring chunks are loaded during selection or
rendering.
...
compact, complex, and mega dungeons, bounded cave pockets, enormous,
flooded, overgrown, ancient, and corrupted caves, a massive hall, and
distinct zombie, skeleton, illager, piglin, and wither-skeleton outposts.
Each compound reserves its complete multi-cell bounds before ordinary content
is rendered, opens only deterministic door connectors, applies region
processors, attaches vanilla or Labrinth-owned loot tables, and populates each
deterministic entity position from its owning chunk without duplicating mobs on
reload. Cave compounds use a
seed-derived filled volume with irregular chambers, tunnels, columns, hazards,
and contained points of interest rather than a rectangular room shell.

The reusable `LabrinthTemplatePiece`, `LabrinthPiecePool`, and
`LabrinthProcessorSet` contracts keep future authored templates, weighted
content pools, and regional transformations on the same ownership path. A
bounded debug lookup, `/labrinth inspect`, `/labrinth stats`, and F3 generator
summary expose selected compounds without changing normal generation.
...
floor, logical depth, discovery tier, region, and entrances; it does not
force-load or alter generation. `/labrinth stats 32` reports selected content
by tier and average sector distance. Existing chunks are not rewritten when
the mod updates, so test the reported destination in an unexplored area or
create a fresh world with the current jar.
